(ns cdr-australia.main
  "Kotodama WASM entrypoint for the cdr_australia clean-room actor (L4) — Clojure port.
  ...
  facts (`cdr_australia.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "cdr_australia")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"cdr_australia.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "cdr_australia-compat" :tier tier :entities entities} 200])
...
(defn start! [] :cdr_australia-compat/ready)
